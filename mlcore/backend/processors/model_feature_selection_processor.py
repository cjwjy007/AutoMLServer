...
class ModelFeatureSelectionProcessor(Processor):
    ...
    def set_attr(self, attr):
        ...
        if 'strategy' in attr:
            self._strategy = attr.get('strategy')
    ...
    def process(self, data):
        ...
        if self._selected_feature is None:
            ...
            if self._strategy == 'model':
                ...
                mask = SelectFromModel(RandomForestClassifier(n_estimators=15), threshold='1.05*mean') \
                    .fit(X, y).get_support()
            elif self._strategy == 'auc_roc':
                mask = self.auc_select(train_x=X, train_y=y)
            self._selected_feature = np.insert(mask, data.columns.tolist().index(self._label), True)
            data = data.iloc[:, self._selected_feature]
        else:
            data = data.iloc[:, self._selected_feature]
        return data

    def auc_select(self, auc_feature_num=5, train_x=None, train_y=None):
        ...
        for k, feature in enumerate(x_train.columns):
            ...
            valid_auc = auc(y_valid, very_simple_learner.predict_proba(valid_input_feature)[:, 1], reorder=True)
        ...
        num_features_in_combination = auc_feature_num
        ...
        for combination in range(num_combinations):
            ...
            # store AUC results
            valid_auc = auc(y_valid, weak_learner.predict_proba(valid_input_features)[:, 1], reorder=True)
        ...
        for comb_i in range(num_combinations):
            for comb_j in range(comb_i + 1, num_combinations):
                # get the features list for each combination
                featuresComb_i = [feature_combinations_table.ix[comb_i, feature_column_names[x]] for x in
                                  range(num_features_in_combination)]
                featuresComb_j = [feature_combinations_table.ix[comb_j, feature_column_names[x]] for x in
                                  range(num_features_in_combination)]
                # store the number of overlapping features
                combination_overlap_matrix[comb_i, comb_j] = 2 * num_features_in_combination - len(
                    set(featuresComb_i + featuresComb_j))
                combination_overlap_matrix[comb_j, comb_i] = combination_overlap_matrix[comb_i, comb_j]
        ...
        column_df = feature_combinations_table.ix[final_features_to_keep, :].drop("combinedAUC", axis=1)
        selected_column = column_df.as_matrix()[0]
